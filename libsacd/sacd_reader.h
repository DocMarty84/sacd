...
enum area_id_e
{
    AREA_BOTH = 0, AREA_TWOCH = 1, AREA_MULCH = 2
};
enum frame_type_e
{
    FRAME_DSD = 0, FRAME_DST = 1, FRAME_INVALID = -1
};
enum media_type_t
{
    UNK_TYPE = 0, ISO_TYPE = 1, DSDIFF_TYPE = 2, DSF_TYPE = 3
};
...
{
public:
    sacd_reader_t() = default;

    virtual ~sacd_reader_t() = default;

    virtual int open(sacd_media_t *p_file) = 0;

    virtual bool close() = 0;

    virtual uint32_t get_track_count(area_id_e area_id) = 0;
