...
{
    return ((float) (m_track_current_lsn - m_track_start_lsn) * 100.0f) / (float) m_track_length_lsn;
}
    ...
    if (track_number < get_track_count(area_id)) {
        ...
        if (!area->area_track_text[track_number].track_type_performer.empty()) {
            ...
            sprintf(buf, "(%ich) %.2i. %s - %s.wav", m_channel_count, track_number + 1,
                    area->area_track_text[track_number].track_type_performer.data(),
                    area->area_track_text[track_number].track_type_title.data());
        } else {
            buf = (char *) calloc(6 + 2 + 2 + area->area_track_text[track_number].track_type_title.size() + 4 + 1, 1);
            sprintf(buf, "(%ich) %.2i. %s.wav", m_channel_count, track_number + 1,
                    area->area_track_text[track_number].track_type_title.data());
        }
            ...
            case DATA_TYPE_AUDIO:
                if (m_frame.started) {
                    if (packet->frame_start) {
                        ...
                        *frame_type =
                                m_sector_bad_reads > 0 ? FRAME_INVALID : m_frame.dst_encoded ? FRAME_DST : FRAME_DSD;
                ...
                if (m_frame.started) {
                    if (m_frame.size + packet->packet_length <= (int) (*frame_size) &&
                        ...
                        for (uint8_t j = 0; j < track_amount; j++) {
                            ...
                            if (*track_ptr != 0) {
                                ...
                                case TRACK_TYPE_TITLE:
                                    area->area_track_text[i].track_type_title = charset_convert(track_ptr,
                                                                                                strlen(track_ptr),
                                                                                                current_charset);
                                    break;
                                case TRACK_TYPE_PERFORMER:
                                    area->area_track_text[i].track_type_performer = charset_convert(track_ptr,
                                                                                                    strlen(track_ptr),
                                                                                                    current_charset);
                                    break;
                                case TRACK_TYPE_SONGWRITER:
                                    area->area_track_text[i].track_type_songwriter = charset_convert(track_ptr,
                                                                                                     strlen(track_ptr),
                                                                                                     current_charset);
                                    break;
                                case TRACK_TYPE_COMPOSER:
                                    area->area_track_text[i].track_type_composer = charset_convert(track_ptr,
                                                                                                   strlen(track_ptr),
                                                                                                   current_charset);
                                    break;
                                case TRACK_TYPE_ARRANGER:
                                    area->area_track_text[i].track_type_arranger = charset_convert(track_ptr,
                                                                                                   strlen(track_ptr),
                                                                                                   current_charset);
                                    break;
                                case TRACK_TYPE_MESSAGE:
                                    area->area_track_text[i].track_type_message = charset_convert(track_ptr,
                                                                                                  strlen(track_ptr),
                                                                                                  current_charset);
                                    break;
                                case TRACK_TYPE_EXTRA_MESSAGE:
                                    area->area_track_text[i].track_type_extra_message = charset_convert(track_ptr,
                                                                                                        strlen(track_ptr),
                                                                                                        current_charset);
                                    break;
                                case TRACK_TYPE_TITLE_PHONETIC:
                                    area->area_track_text[i].track_type_title_phonetic = charset_convert(track_ptr,
                                                                                                         strlen(track_ptr),
                                                                                                         current_charset);
                                    break;
                                case TRACK_TYPE_PERFORMER_PHONETIC:
                                    area->area_track_text[i].track_type_performer_phonetic = charset_convert(track_ptr,
                                                                                                             strlen(track_ptr),
                                                                                                             current_charset);
                                    break;
                                case TRACK_TYPE_SONGWRITER_PHONETIC:
                                    area->area_track_text[i].track_type_songwriter_phonetic = charset_convert(track_ptr,
                                                                                                              strlen(track_ptr),
                                                                                                              current_charset);
                                    break;
                                case TRACK_TYPE_COMPOSER_PHONETIC:
                                    area->area_track_text[i].track_type_composer_phonetic = charset_convert(track_ptr,
                                                                                                            strlen(track_ptr),
                                                                                                            current_charset);
                                    break;
                                case TRACK_TYPE_ARRANGER_PHONETIC:
                                    area->area_track_text[i].track_type_arranger_phonetic = charset_convert(track_ptr,
                                                                                                            strlen(track_ptr),
                                                                                                            current_charset);
                                    break;
                                case TRACK_TYPE_MESSAGE_PHONETIC:
                                    area->area_track_text[i].track_type_message_phonetic = charset_convert(track_ptr,
                                                                                                           strlen(track_ptr),
                                                                                                           current_charset);
                                    break;
