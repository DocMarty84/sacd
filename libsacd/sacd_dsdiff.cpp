...
uint32_t sacd_dsdiff_t::get_track_count(area_id_e area_id)
{
    if ((area_id == AREA_TWOCH && m_channel_count == 2) || (area_id == AREA_MULCH && m_channel_count > 2) ||
        area_id == AREA_BOTH) {
        return static_cast<uint32_t>(m_subsong.size());
    }
...
{
    return ((float) (m_file->get_position() - m_current_offset) * 100.0f) / (float) m_current_size;
}
...
int sacd_dsdiff_t::open(sacd_media_t *p_file)
{
        ...
        if (ck.has_id("FVER") && ck.get_size() == 4) {
            ...
            while (id_prop_read < id_prop_size) {
                ...
                if (ck.has_id("FS  ") && ck.get_size() == 4) {
                    ...
                    m_channel_count = static_cast<uint16_t>(hton16(channel_count));
                    ...
                    t_old.offset = static_cast<uint64_t>(m_file->get_position());
            ...
            m_dsti_offset = static_cast<uint64_t>(m_file->get_position());
            ...
            while (id_diin_read < id_diin_size) {
                ...
                if (ck.has_id("MARK") && ck.get_size() >= sizeof(Marker)) {
                    ...
                    if (m_file->read(&m, sizeof(Marker)) == sizeof(Marker)) {
                        m.hours = static_cast<uint16_t>(hton16(m.hours));
                        m.samples = hton32(m.samples);
                        m.offset = hton32(m.offset);
                        m.markType = static_cast<uint16_t>(hton16(m.markType));
                        m.markChannel = static_cast<uint16_t>(hton16(m.markChannel));
                        m.TrackFlags = static_cast<uint16_t>(hton16(m.TrackFlags));
                        ...
                        case TrackStart: {
                            ...
                            if (!m_subsong.empty()) {
                                ...
                                if (m_subsong.size() - 1 > 0) {
                                    if (m_subsong[m_subsong.size() - 2].stop_time >
                                        m_subsong[m_subsong.size() - 1].start_time) {
                                        m_subsong[m_subsong.size() - 2].stop_time = m_subsong[m_subsong.size() -
                                                                                              1].start_time;
                                    }
                                }
                            }
            ...
            id3tags_t t;
            m_id3tags.size();
            t.offset = static_cast<uint64_t>(m_file->get_position());
            t.size = ck.get_size();
    ...
    return static_cast<int>(m_subsong.size());
}
...
bool sacd_dsdiff_t::read_frame(uint8_t *frame_data, size_t *frame_size, frame_type_e *frame_type)
{
    if (m_dst_encoded) {
        ...
        while ((uint64_t) m_file->get_position() < m_current_offset + m_current_size &&
        ...
        auto position = static_cast<uint64_t>(m_file->get_position());
        *frame_size = (size_t) MIN((int64_t) m_frame_size, (int64_t) MAX(0,
                                                                         (int64_t) (m_current_offset + m_current_size) -
                                                                         (int64_t) position));
...
uint64_t sacd_dsdiff_t::get_dsti_for_frame(uint32_t frame_nr)
{
    ...
    cur_offset = static_cast<uint64_t>(m_file->get_position());
