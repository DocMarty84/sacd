...
int ReadDSDframe(StrData* SD, long MaxFrameLen, int NrOfChannels, uint8_t* DSDFrame);
int RiceDecode(StrData* SD, int m);
int Log2RoundUp(long x);
int ReadTableSegmentData(StrData* SD, int NrOfChannels, int FrameLen, int MaxNrOfSegs, int MinSegLen, Segment* S, int* SameSegAllCh);
int CopySegmentData(FrameHeader* FH);
int ReadSegmentData(StrData* SD, FrameHeader* FH);
int ReadTableMappingData(StrData* SD, int NrOfChannels, int MaxNrOfTables, Segment* S, int* NrOfTables, int* SameMapAllCh);
int CopyMappingData(FrameHeader* FH);
int ReadMappingData(StrData* SD, FrameHeader* FH);
int ReadFilterCoefSets(StrData* SD, int NrOfChannels, FrameHeader* FH, CodedTableF* CF);
int ReadProbabilityTables(StrData* SD, FrameHeader* FH, CodedTableP* CP, int P_one[2 * MAX_CHANNELS][AC_HISMAX]);
int ReadArithmeticCodedData(StrData* SD, int ADataLen, uint8_t* AData);

/*
name: ReadDSDframe
function: Read DSD signal of this frame from the DST input file.
pre: a file must be opened by using getbits_init(), MaxFrameLen, NrOfChannels
post: BS11[][]
*/

int ReadDSDframe(StrData* S, long MaxFrameLen, int NrOfChannels, uint8_t* DSDFrame)
{
    int ByteNr;
...
/*
name: Log2RoundUp
function: Calculate the log2 of an integer and round the result up, by using integer arithmetic.
pre: x
...
pre: FH->NrOfChannels, FH->FSeg.Resolution, FH->FSeg.NrOfSegments[], FH->FSeg.SegmentLen[][]
post: FH->: PSeg: .Resolution, .NrOfSegments[], .SegmentLen[][], PSameSegAllCh
*/
...
post: FH->: FSeg: .Resolution, .SegmentLen[][], .NrOfSegments[], PSeg: .Resolution, .SegmentLen[][], .NrOfSegments[], PSameSegAsF, FSameSegAllCh, PSameSegAllCh
*/
...
pre: CO->: NrOfChannels, MaxNrOfPtables FH->: FSeg.NrOfSegments[], FSeg.Table4Segment[][], NrOfFilters, PSeg.NrOfSegments[]
...
int CopyMappingData(FrameHeader* FH)
{
    ...
    for (ChNr = 0; ChNr < FH->NrOfChannels; ChNr++)
    {
        ...
        {
            printf("PANIC: Not same number of segments for filters and Ptables!\n");
            return -1;
        }
    }
...
pre: CO->: NrOfChannels, MaxNrOfFilters, MaxNrOfPtables FH->: FSeg.NrOfSegments[], PSeg.NrOfSegments[]
post: FH->: FSeg.Table4Segment[][], .NrOfFilters, PSeg.Table4Segment[][], .NrOfPtables, PSameMapAsF, FSameMapAllCh, PSameMapAllCh, HalfProb[]
*/
...
~ prediction order
~ all coefficients
pre: a file must be opened by using getbits_init(), NrOfChannels FH->NrOfFilters, CF->CPredOrder[], CF->CPredCoef[][], FH->FSeg.Table4Segment[][0]
post: FH->PredOrder[], FH->ICoefA[][], FH->NrOfHalfBits[], CF->Coded[], CF->BestMethod[], CF->m[][],
*/
...
pre: a file must be opened by using getbits_init(), FH->NrOfPtables, CP->CPredOrder[], CP->CPredCoef[][]
post: FH->PtableLen[], CP->Coded[], CP->BestMethod[], CP->m[][], P_one[][]
*/
...
int ReadArithmeticCodedData(StrData* SD, int ADataLen, ADataByte* AData)
{
    int j;
    int return_value;
    uint8_t Val;
...
int UnpackDSTframe(DstDec* D, uint8_t* DSTdataframe, uint8_t* DSDdataframe)
{
    ...
    else
    {
        ...
        {
            printf("PANIC: Illegal arithmetic code in frame %d!\n", D->FrameHdr.FrameNr);
            return -1;
        }
    }
