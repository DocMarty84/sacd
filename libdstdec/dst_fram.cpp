...
void LT_ACDecodeBit_Init(ACData* AC, ADataByte* cb, int fs)
{
    AC->Init = 0;
    AC->A = ONE - 1;
    AC->C = 0;
...
void LT_ACDecodeBit_Decode(ACData* AC, uint8_t* b, int p, ADataByte* cb, int fs)
{
    unsigned int ap;
    unsigned int h;
...
void LT_ACDecodeBit_Flush(ACData* AC, uint8_t* b, int p, ADataByte* cb, int fs)
{
    AC->Init = 1;
...
int LT_ACGetPtableIndex(int16_t PredicVal, int PtableLen)
{
...
/*
    name: FillTable4Bit
    function: Fill an array that indicates for each bit of each channel which table number must be used.
    pre: NrOfChannels, NrOfBitsPerCh, S->NrOfSegments[], S->SegmentLen[][], S->Resolution, S->Table4Segment[][]
    post: Table4Bit[][]
*/

void FillTable4Bit(int NrOfChannels, int NrOfBitsPerCh, Segment* S, uint8_t Table4Bit[MAX_CHANNELS][MAX_DSDBITS_INFRAME / 2])
{
    int BitNr;
    int ChNr;
    int SegNr;
...
/*
    name: Reverse7LSBs
    function: Take the 7 LSBs of a number consisting of SIZE_PREDCOEF bits (2's complement), reverse the bit order and add 1 to it.
    pre: c
...
static void LT_InitCoefTablesI(DstDec*D, int16_t ICoefI[2 * MAX_CHANNELS][16][256])
{
...
/*static void LT_InitCoefTablesU(DstDec* D, uint16_t ICoefU[2 * MAX_CHANNELS][16][256]) {
    int FilterNr, FilterLength, TableNr, k, i, j;
    for (FilterNr = 0; FilterNr < D->FrameHdr.NrOfFilters; FilterNr++) {
        FilterLength = D->FrameHdr.PredOrder[FilterNr];
        for (TableNr = 0; TableNr < 16; TableNr++) {
...
static void LT_InitStatus(DstDec* D, uint8_t Status[MAX_CHANNELS][16])
{
    int ChNr, TableNr;
...
/*
    ...
    pre: D->CodOpt: .NrOfBitsPerCh, .NrOfChannels, D->FrameHdr: .PredOrder[], .NrOfHalfBits[], .ICoefA[][], .NrOfFilters, .NrOfPtables, .FrameNr D->P_one[][], D->AData[], D->ADataLen,
    post: D->WM.Pwm
*/
...
int DST_FramDSTDecode(DstDec* D, uint8_t* DSTdata, uint8_t* MuxedDSDdata, int FrameSizeInBytes, int FrameCnt)
{
    ...
    if (D->FrameHdr.DSTCoded == 1)
    {
        ...
        dst_memset(MuxedDSD, 0, NrOfBitsPerCh * NrOfChannels / 8);
